...
['TYPEFACE_NOTE_TEMPLATE','Language-specific pixel typefaces are included by default at {bitmapSizes} dots. Website typefaces +{addOn} include EB Garamond and the site’s Chinese/Japanese fonts at {siteSizes} dots. A published translation +{addOn} is available for works already translated into the selected language.','各语言的点阵字体均默认包含，使用 {bitmapSizes} 点整倍字形。网站字体 +{addOn}，包含 EB Garamond 与网站的中日文字体，提供 {siteSizes} 点。附加已发布译文 +{addOn}，仅提供已有对应语言译文的作品。','言語別のドット書体は標準で含まれ、{bitmapSizes} ドットで使用します。サイトの書体は +{addOn} で、EB Garamond とサイトの中国語・日本語書体を {siteSizes} ドットで使用します。公開済みの翻訳は +{addOn} で、該当言語の訳がある作品のみ利用できます。','Sprachspezifische Pixelschriften sind standardmäßig enthalten und werden mit {bitmapSizes} Punkten verwendet. Website-Schriften kosten +{addOn} und umfassen EB Garamond sowie die chinesischen/japanischen Schriften der Website in {siteSizes} Punkten. Eine veröffentlichte Übersetzung kostet +{addOn} und ist nur für Werke mit einer Fassung in der gewählten Sprache verfügbar.','Les polices matricielles propres à chaque langue sont incluses par défaut en {bitmapSizes} points. Les polices du site ajoutent +{addOn} et comprennent EB Garamond ainsi que les polices chinoises/japonaises du site en {siteSizes} points. Une traduction publiée ajoute +{addOn}, uniquement si une version existe dans la langue choisie.','Языковые пиксельные шрифты включены по умолчанию и используются в размере {bitmapSizes} точек. Шрифты сайта стоят +{addOn} и включают EB Garamond, а также китайские/японские шрифты сайта в размере {siteSizes} точек. Опубликованный перевод стоит +{addOn} и доступен только для произведений с версией на выбранном языке.'],
...
['TARIFF_NOTE_TEMPLATE','{version}: the poem is weighted by characters, non-empty lines and stanzas, then rounded up to the next price ending in {ending}. Punctuation, spaces, title and byline are excluded from character counts. Website typefaces, a published translation and custom text each add {addOn}. Writing your own or editing an original counts as custom text. Translation has no extra character charge; items sum to the total.','{version}：原文按字数、非空行和分节加权，向上调至最近的 {ending} 结尾价格。标点、空格、题名及署名不计字数。网站字体、已发布译文及自选内容各加 {addOn}；自写或修改原作即属自选内容。译文不重复计字费，各商品相加为总价。','{version}：文字数、空白でない行、連で加重計算し、{ending} で終わる次の価格に切り上げます。句読点、空白、題名、署名は文字数に含めません。サイト書体・公開済みの翻訳・カスタム本文は各 +{addOn}。自作や原作の編集はカスタム本文です。翻訳の文字料金は重複せず、各商品の和が合計です。','{version}: Zeichen, nicht leere Zeilen und Strophen bestimmen den Preis; aufgerundet wird auf den nächsten Preis mit der Endung {ending}. Satzzeichen, Leerzeichen, Titel und Name zählen nicht als Zeichen. Website-Schriften, Übersetzung und eigener Text kosten jeweils +{addOn}. Eigene oder bearbeitete Texte gelten als eigener Text. Keine weitere Zeichengebühr für die Übersetzung; der Gesamtpreis ist die Artikelsumme.','{version} : le prix est pondéré par caractères, lignes non vides et strophes, puis arrondi au prix supérieur se terminant par {ending}. Ponctuation, espaces, titre et signature sont exclus du nombre de caractères. Polices du site, traduction publiée et texte personnalisé ajoutent chacun +{addOn}. Écrire ou modifier une œuvre compte comme texte personnalisé. Aucun second calcul par caractère pour la traduction ; le total est la somme des articles.','{version}: цена зависит от числа символов, непустых строк и строф и округляется вверх до следующей цены с окончанием {ending}. Пунктуация, пробелы, название и подпись не входят в число символов. Шрифты сайта, опубликованный перевод и собственный текст добавляют по +{addOn}. Новый или изменённый текст считается собственным. Перевод не оплачивается посимвольно повторно; итог — сумма позиций.'],
...
const traditionalOverrides={
  ...
  'TYPEFACE_NOTE_TEMPLATE':'各語言的點陣字體均預設包含，使用 {bitmapSizes} 點整倍字形。網站字體 +{addOn}，包含 EB Garamond 與網站的中日文字體，提供 {siteSizes} 點。附加已發表譯文 +{addOn}，僅提供已有對應語言譯文的作品。',
  'TARIFF_NOTE_TEMPLATE':'{version}：原文按字數、非空行和分節加權，向上調至最近的 {ending} 結尾價格。標點、空格、題名及署名不計字數。網站字體、已發表譯文及自選內容各加 {addOn}；自寫或修改原作即屬自選內容。譯文不重複計字費，各商品相加為總價。',
