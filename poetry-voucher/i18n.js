...
['TYPEFACE_NOTE_TEMPLATE','Language-specific pixel typefaces are included by default at {bitmapSizes} dots. Website typefaces +{addOn} include EB Garamond and the site’s Chinese/Japanese fonts at {siteSizes} dots. English translation +{addOn} is available only for works with an existing published translation.','各语言的点阵字体均默认包含，使用 {bitmapSizes} 点整倍字形。网站字体 +{addOn}，包含 EB Garamond 与网站的中日文字体，提供 {siteSizes} 点。附加英译 +{addOn}，仅提供已有原站英译的作品。','言語別のドット書体は標準で含まれ、{bitmapSizes} ドットで使用します。サイトの書体は +{addOn} で、EB Garamond とサイトの中国語・日本語書体を {siteSizes} ドットで使用します。英訳は +{addOn} で、公開済みの英訳がある作品のみ利用できます。','Sprachspezifische Pixelschriften sind standardmäßig enthalten und werden mit {bitmapSizes} Punkten verwendet. Website-Schriften kosten +{addOn} und umfassen EB Garamond sowie die chinesischen/japanischen Schriften der Website in {siteSizes} Punkten. Eine englische Übersetzung kostet +{addOn} und ist nur bei bereits veröffentlichter Übersetzung verfügbar.','Les polices matricielles propres à chaque langue sont incluses par défaut en {bitmapSizes} points. Les polices du site ajoutent +{addOn} et comprennent EB Garamond ainsi que les polices chinoises/japonaises du site en {siteSizes} points. La traduction anglaise ajoute +{addOn}, uniquement lorsqu’une traduction publiée existe déjà.','Языковые пиксельные шрифты включены по умолчанию и используются в размере {bitmapSizes} точек. Шрифты сайта стоят +{addOn} и включают EB Garamond, а также китайские/японские шрифты сайта в размере {siteSizes} точек. Английский перевод стоит +{addOn} и доступен только для произведений с уже опубликованным переводом.'],
...
['TARIFF_NOTE_TEMPLATE','{version}: the poem is weighted by characters, non-empty lines and stanzas, then rounded up to the next price ending in {ending}. Punctuation, spaces, title and byline are excluded from character counts. Website typefaces, English translation and custom text each add {addOn}. Writing your own or editing an original counts as custom text. Translation has no extra character charge; items sum to the total.','{version}：原文按字数、非空行和分节加权，向上调至最近的 {ending} 结尾价格。标点、空格、题名及署名不计字数。网站字体、英译及自选内容各加 {addOn}；自写或修改原作即属自选内容。英译不重复计字费，各商品相加为总价。','{version}：文字数、空白でない行、連で加重計算し、{ending} で終わる次の価格に切り上げます。句読点、空白、題名、署名は文字数に含めません。サイト書体・英訳・カスタム本文は各 +{addOn}。自作や原作の編集はカスタム本文です。英訳の文字料金は重複せず、各商品の和が合計です。','{version}: Zeichen, nicht leere Zeilen und Strophen bestimmen den Preis; aufgerundet wird auf den nächsten Preis mit der Endung {ending}. Satzzeichen, Leerzeichen, Titel und Name zählen nicht als Zeichen. Website-Schriften, Übersetzung und eigener Text kosten jeweils +{addOn}. Eigene oder bearbeitete Texte gelten als eigener Text. Keine weitere Zeichengebühr für die Übersetzung; der Gesamtpreis ist die Artikelsumme.','{version} : le prix est pondéré par caractères, lignes non vides et strophes, puis arrondi au prix supérieur se terminant par {ending}. Ponctuation, espaces, titre et signature sont exclus du nombre de caractères. Polices du site, traduction anglaise et texte personnalisé ajoutent chacun +{addOn}. Écrire ou modifier une œuvre compte comme texte personnalisé. Aucun second calcul par caractère pour la traduction ; le total est la somme des articles.','{version}: цена зависит от числа символов, непустых строк и строф и округляется вверх до следующей цены с окончанием {ending}. Пунктуация, пробелы, название и подпись не входят в число символов. Шрифты сайта, английский перевод и собственный текст добавляют по +{addOn}. Новый или изменённый текст считается собственным. Перевод не оплачивается посимвольно повторно; итог — сумма позиций.'],
...
const traditionalOverrides={
  ...
  'TYPEFACE_NOTE_TEMPLATE':'各語言的點陣字體均預設包含，使用 {bitmapSizes} 點整倍字形。網站字體 +{addOn}，包含 EB Garamond 與網站的中日文字體，提供 {siteSizes} 點。附加英譯 +{addOn}，僅提供已有原站英譯的作品。',
  'TARIFF_NOTE_TEMPLATE':'{version}：原文按字數、非空行和分節加權，向上調至最近的 {ending} 結尾價格。標點、空格、題名及署名不計字數。網站字體、英譯及自選內容各加 {addOn}；自寫或修改原作即屬自選內容。英譯不重複計字費，各商品相加為總價。',
