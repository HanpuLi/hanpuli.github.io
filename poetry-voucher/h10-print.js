...
(() => {
  ...
  function onMessage(event){
    ...
    if(data.type==='ready'&&typeof data.nonce==='string'&&/^[0-9a-f-]{36}$/.test(data.nonce)){
      bridge={nonce:data.nonce,origin:event.origin,protocol:data.protocol===2?2:1};
      for(const button of attached)button.hidden=button.dataset.requiresV2==='true'&&bridge.protocol<2;
      return;
    }
    if(data.type==='print-result'&&bridge&&data.nonce===bridge.nonce&&/^[0-9]{12}$/.test(data.reference)){
      for(const button of attached){
        if(button.dataset.reference!==data.reference||button.dataset.orderKey!==(data.id||'web-'+data.reference))continue;
        if(typeof data.accepted!=='boolean')continue;
        update(button,data.accepted?'accepted':'interrupted');
      }
    }
  }
  ...
  window.h10OrderPrinter={
    add(button,order,result){
      ...
      button.dataset.orderKey=PoetryOrder.printIdentity(order);button.dataset.requiresV2=String(!!order.id);
      button.hidden=!bridge||(!!order.id&&bridge.protocol<2);
      update(button,'');
      button.addEventListener('click',()=>{
        if(!bridge||window.parent===window||button.disabled||(order.id&&bridge.protocol<2))return;
        try{
          const orderPages=window.poetryShop?.pagesForH10?.(PoetryOrder.identity(order),result);
          if(!Array.isArray(orderPages)||orderPages.length<2)throw Error('order pages unavailable');
          const packed=PoetryOrder.pack(orderPages);
          if(packed.reduce((total,page)=>total+page.height,0)>120000)throw Error('order too long');
          update(button,'sending');
          window.parent.postMessage({channel:CHANNEL,type:'print-order',nonce:bridge.nonce,id:PoetryOrder.printIdentity(order),reference:order.ref,pages:packed},bridge.origin);
        }catch(error){
