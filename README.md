...
## First Love public reading

The seven-language First Love overview and the former `/status/` and `/read/` URLs are generated from `content/first-love-public.json` by `tools/first_love_public_pages.py`. The home-page reading link and all 21 First Love pages open the same anonymous reader URL. The legacy URLs remain as noindex landing pages so old bookmarks do not expose a token or PDF client. `tools/firstlovecheck.py` verifies the direct links, locked-down page CSP and absence of the old access script and manuscript files in the public tree. The separate reviewed reader went live at the pinned URL on 24 September 2026; the former whole-PDF service and routes were retired.
