...
try{
 ...
 // Add another order without invalidating the first order's downloads.
 const firstHref=await page.locator('.order-record a').first().getAttribute('href');await add(1);await bag();await page.locator('#to-checkout').click();await page.locator('#place-order').click();await page.waitForFunction(()=>window.poetryShop.snapshot().orders.length===2,{},{timeout:30000});assert.equal(await page.locator('.completed-order').count(),2);assert.equal(await page.locator(`a[href="${firstHref}"]`).count(),1);const oldDownload=page.waitForEvent('download');await page.locator(`a[href="${firstHref}"]`).click();assert((await readFile(await (await oldDownload).path())).length>1000);
 ...
 for(const lang of ['en','zh-Hant','zh-Hans','ja','de','fr','ru']){
  ...
  for(const width of [320,390,768,1440]){await page.setViewportSize({width,height:950});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),`${lang} ${width} overflow`);}
 }
 ...
 await stress.locator('#place-order').click();await stress.waitForFunction(()=>poetryShop.snapshot().orders.length===1,{},{timeout:60000});assert.equal(await stress.locator('.voucher-proof').count(),24);assert.equal(new Set(await stress.locator('.voucher-proof>.micro').allTextContents()).size,24);await stressContext.close();
 assert.deepEqual(errors,[]);console.log('shopcheck: cart snapshots, variants, edit cancellation, quantity, persistence, custom-text opt-in, locale stability, exactly-once checkout, PDF pages, multiple orders, corrupt storage, 28 viewports and 4 axe surfaces OK');
