...
try{
 ...
 await bag();await page.locator('.bag-line').getByRole('button',{name:'Edit',exact:true}).click();assert.equal(await page.locator('#title').inputValue(),'A long private poem');assert.equal((await page.locator('#poem').inputValue()).length,1750);await page.locator('#save-line').click();await page.locator('#save-custom').check();saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('poetry-voucher-bag-v1')));assert.equal(saved.lines.length,1);await close('bag-dialog');await page.reload();await page.waitForFunction(()=>document.querySelectorAll('.product-card').length===23);assert.equal((await state()).bag.length,1);
 ...
 const multiOrder=(await multi.evaluate(()=>poetryShop.snapshot())).orders[0];assert.equal(multiOrder.lines[0].items.filter(item=>item.id==='translation').length,5);assert.equal(await multi.locator('.order-reading .verse').count(),6);assert(await multi.locator('.voucher-proof img').count()>=1);await multiContext.close();
